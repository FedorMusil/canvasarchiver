...
import { exampleModulesItems } from './moduleitems-mock';
import { ModuleItemType } from '../api/moduleItem';
...
export const exampleModules: Module[] = [
    {
        ...
        items_count: exampleModulesItems.length,
    ...
    },
    {
        ...
        position: 1,
        name: "Assignments",
        unlock_at: "2012-12-01T06:00:00-06:00",
        require_sequential_progress: false,
        prerequisite_module_ids: [],
        items_count: exampleModulesItems.length,
        items_url: "https://canvas.example.com/api/v1/modules/124/items",
        items: 
        [
            {
                id: 770,
...
export const moduleHandlers: HttpHandler[] = [
    ...
    http.get(`${import.meta.env.VITE_BACKEND_URL}/modules/:id`, (req) => {
        const { id } = req.params;
        const module = exampleModules.find((module: { id: number; }) => module.id === parseInt(id, 10));
        if (module) {
            return HttpResponse.json<Module>(module);
        }
    }),
    // retrieve all modules
    http.get(`${import.meta.env.VITE_BACKEND_URL}/modules`, () => {
        return HttpResponse.json<Module[]>(exampleModules);
    }),
];