...
const QuizRightContainer: React.FC<QuizContainerProps> = ({ changeBefore, changeAfter }: QuizContainerProps) => {
    ...
    function compareQuizzes(oldData: Quiz[], newData: Quiz[]): Quiz[] {
        const added = newData
            .filter((nq) => !oldData.some((oq) => oq.id === nq.id))
            .map((quiz) => ({ ...quiz, status: 'added' }));
        const removed = oldData
            .filter((oq) => !newData.some((nq) => nq.id === oq.id))
        ...
        const modified = existing
            .map((nq) => {
                ...
                if (
                    nq.title !== oq.title ||
                    nq.question_count !== oq.question_count ||
                    nq.points_possible !== oq.points_possible ||
                    new Date(nq.due_at).getTime() !== new Date(oq.due_at).getTime() ||
        ...
        const unchanged = existing
            .filter((eq) => !modified.some((mq) => mq && mq.id === eq.id))
    ...
    return (
        ...
        >
            <div style={{ backgroundColor: '#f3f3f3', width: '100%', maxWidth: '100%', border: '1px solid #ccc' }}>
                ...
                >
                    <span style={{ fontSize: '19px', fontWeight: 'bold', color: '#202020' }}>
                        {isOpen ? '▾' : '▸'} Assignment quizzes
                    </span>
                </div>
                {isOpen &&
                    quizzes.map((quiz, index) => (
                        <div
                            ...
                            style={{
                                ...
                                backgroundColor: getBackgroundColor(quiz),
                                paddingLeft: '20px',
                                paddingBottom: '14px',
                                color: '#202020',
                            }}
                            ...
                            <div style={{ display: 'flex', gap: '15px', fontSize: '15px' }}>
                                {new Date() >= new Date(quiz.unlock_at) && new Date() <= new Date(quiz.due_at) ?
                                    <span style={{ color: '#454545' }}>
                                        <span style={{ fontWeight: 'bold' }}>Available until</span>{' '}
                                ...
                                :   <span style={{ color: '#454545' }}>
                                        ...
                                        {formatDate(quiz.unlock_at)}
                                    </span>
                                }
                                <span style={{ color: '#454545' }}>
                                    <span style={{ fontWeight: 'bold' }}>Due</span> {formatDate(quiz.due_at)}
                                </span>
