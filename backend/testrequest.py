...
def create_course(course_content = {"name": "New Course","course_code": '127457'}):
...
def check_course_create(course_id):
    check_course_create_url = "http://localhost:5000/course/{}/id".format(course_id)
...
def create_change(course_id, change_data={"change_type": "Deletion", "item_type": "Assignments", "old_value": "0", "new_value": "{'name': 'New Course', 'course_code': '123457'}"}):
...
def create_annotation(course_id, change_id, annotation_data={"change_id": "1", "user_id": "1", "text": "This is an annotation"}):
...
def get_all_changes(course_id):
    check_change_create_url = "http://localhost:5000/course/{}/changes".format(course_id)
    response = requests.get(check_change_create_url)
    return response.json()

def get_all_annotations(course_id, change_id):
    check_annotation_create_url = "http://localhost:5000/course/{}/annotations/{}".format(course_id, change_id)
...
def create_and_check_user(course_id):
    ...
    user_role = "Teacher"
    id = create_user(course_id, {"email": user_email, "name": user_name, "role": user_role})
...
if __name__ == "__main__":
    ...
    # Change generation
    change_id = create_change(course_id.get("course_id"), {"course_id":course_id.get("course_id"), "change_type": "Deletion", "item_type": "Assignments", "old_value": "0", "new_value": json.dumps({"name": "New Course", "course_code": "123457"})})
    ...
    for i in range (len(changes)):
        if changes[i][0] == change_id.get("change_id"):
            ...
            assert changes[i][6] == {'name': 'New Course', 'course_code': '123457'}
            change_found = True
            break
    if not change_found:
        assert False

    annotations = get_all_annotations(course_id.get("course_id"), change_id.get("change_id"))
